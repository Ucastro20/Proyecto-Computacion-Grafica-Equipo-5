...
/* ----------------   Práctica 8 --------------------------*/
/*-----------------    2024-2   ---------------------------*/
/*------------- Alumno: Castro Rodriguez Ulises ---------------*/
/*------------- No. Cuenta: 318336466       ---------------*/
...
//Camera
Camera camera(glm::vec3(0.0f, 10.0f, 20.0f)); //Constructor con la posicion inicial de la camara
...
void LoadTextures()
{
	...
	t_Aatrox = generateTextures("Texturas/Aatrox.jpg", 0, true);  //3 parametros: ruta, transparencia, indica si usamos un plano/primitiva
	// o un modelo de 3Ds Max
	//jpg no soporta transparencia, png si
	t_smile = generateTextures("Texturas/awesomeface.png", 1, true);//Se usa t_ en el nombre para cargar texturas generalmente
	...
	t_Dado = generateTextures("Texturas/Dado.jpg", 0, true);
...
void animate(void)
{
	lightPosition.x = 150.0f * sin(anguloCastro);
	lightPosition.z = 150.0f * cos(anguloCastro);
...
void myData() {
	float vertices[] = {
		// positions          // texture coords (s,t)
		 0.5f,  0.5f, 0.0f,   1.0f, 1.0f, // top right //Modificar los "top" para modificar la cantidad de veces que se repite verticalmente
		 0.5f, -0.5f, 0.0f,   1.0f, 0.0f, // bottom right  //Modificar los "right" para modificar la cantidad de veces que se repite horizontalmente
...
int main() {
	...
	// load models
	// -----------
	Model fachada("resources/objects/Fachada/FachadaCentroCom.obj");
	//Model paredes("resources/objects/paredes/Paredes.obj");
	//Model piso("resources/objects/piso/piso.obj"); //Se cargan modelos con la clase Model y la ruta del archivo .obj
	Model carro("resources/objects/lambo/carroceria.obj");
	Model llanta("resources/objects/lambo/Wheel.obj");
	Model casaVieja("resources/objects/casa/OldHouse.obj");
	//Model cubo("resources/objects/cubo/cube02.obj");
	Model casaDoll("resources/objects/casa/DollHouse.obj");
	Model BrazoIzqAquaman("resources/objects/StewieAquaman/BrazoIzq.obj");
	Model TorsoAquaman("resources/objects/StewieAquaman/Torso.obj");
	Model CabezaAquaman("resources/objects/StewieAquaman/Cabeza.obj");
	Model BrazoDerAquaman("resources/objects/StewieAquaman/BrazoDer.obj");
	Model PiernaIzqAquaman("resources/objects/StewieAquaman/PiernaIzq.obj");
	Model PiernaDerAquaman("resources/objects/StewieAquaman/PiernaDer.obj");
	Model BotaIzqAquaman("resources/objects/StewieAquaman/BotaIzq.obj");
	Model BotaDerAquaman("resources/objects/StewieAquaman/BotaDer.obj");
	Model casaBrujas("resources/objects/CasaBrujas/brujas.obj");
	Model cuboCastro("resources/objects/CuboCastro/cuboCastro.obj");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		staticShader.setVec3("pointLight[0].position", lightPosition); //Luz posicional/Puntual .position es para la position de la luz (x,y,z)
		staticShader.setVec3("pointLight[0].ambient", glm::vec3(0.0f, 0.0f, 0.0f));
		staticShader.setVec3("pointLight[0].diffuse", glm::vec3(Red, Green, Blue));
		...
		//myShader.setMat4("model", modelOp);
		//myShader.setVec3("aColor", 1.0f, 1.0f, 1.0f);
		//glBindTexture(GL_TEXTURE_2D, t_Aatrox); //Comando para identificar la textura q utilizaremos
		...
		//modelOp = glm::translate(glm::mat4(1.0f), glm::vec3(-180.0f, 0.0f, 150.0f));
		//modelOp = glm::rotate(modelOp, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		//modelOp = glm::scale(modelOp, glm::vec3(10.0f));
		//staticShader.setMat4("model", modelOp);
		//cuboCastro.Draw(staticShader);
		...
		//modelOp = glm::translate(glm::mat4(1.0f), glm::vec3(-180.0f, 0.0f, 0.0f));
		...
		////Utilizaremos una matriz temporal para el modelado jerarquico
		//glm::mat4 tmpAquaman = glm::mat4(1.0f);
		//glm::mat4 tmpPiernaDer = glm::mat4(1.0f);
		//glm::mat4 tmpPiernaIzq = glm::mat4(1.0f);

		//tmpAquaman = modelOp = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, 0.0f));
		//staticShader.setMat4("model", modelOp);
		//TorsoAquaman.Draw(staticShader);

		//modelOp = glm::translate(tmpAquaman, glm::vec3(0.75f, 1.5f, 0.0f)); //Ya no se construye en el origen, ahora se construye con la matriz temporal
		////que guarda el estado del torso
		//modelOp = glm::rotate(modelOp, glm::radians(rotBrazo), glm::vec3(1.0f, 0.0f, 0.0f)); //Rotacion para la animacion del brazo
		//staticShader.setMat4("model", modelOp);
		//BrazoIzqAquaman.Draw(staticShader);

		//modelOp = glm::translate(tmpAquaman, glm::vec3(0.0f, 1.5f, 0.0f));
		////modelOp = glm::rotate(modelOp, glm::radians(rotBrazo), glm::vec3(0.0f, 1.0f, 0.0f)); //Rotacion de la cabeza movimiento horizontal
		//staticShader.setMat4("model", modelOp);
		...
		//tmpPiernaIzq = modelOp = glm::rotate(modelOp, glm::radians(-rotBrazo), glm::vec3(1.0f, 0.0f, 0.0f));
		//staticShader.setMat4("model", modelOp);
		//PiernaIzqAquaman.Draw(staticShader);

		//modelOp = glm::translate(tmpPiernaIzq, glm::vec3(0.0f, -0.9f, 0.0f));
		//staticShader.setMat4("model", modelOp);
		//BotaIzqAquaman.Draw(staticShader);

		//modelOp = glm::translate(tmpAquaman, glm::vec3(-0.5f, 0.0f, -0.1f));
		//tmpPiernaDer = modelOp = glm::rotate(modelOp, glm::radians(rotBrazo), glm::vec3(1.0f, 0.0f, 0.0f));
		//staticShader.setMat4("model", modelOp);
		//PiernaDerAquaman.Draw(staticShader);

		//modelOp = glm::translate(tmpPiernaDer, glm::vec3(0.0f, -0.9f, 0.0f));
		//staticShader.setMat4("model", modelOp);
		//BotaDerAquaman.Draw(staticShader);


		//Para la practica poner todas las piezas del modelo
...
void my_input(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	//Control de color de luces

	if (glfwGetKey(window, GLFW_KEY_KP_4) == GLFW_PRESS) { //Blanco
	...
	if (glfwGetKey(window, GLFW_KEY_KP_5) == GLFW_PRESS) { //Control de luz direccional
	...
	//if (glfwGetKey(window, GLFW_KEY_KP_5) == GLFW_PRESS) { //Rojo
	//	Red = 1.0f;
	//	Green = 0.0f;
	//	Blue = 0.0f;
	//}

	//if (glfwGetKey(window, GLFW_KEY_KP_6) == GLFW_PRESS) { //Verde
	//	Red = 0.0f;
	//	Green = 1.0f;
	//	Blue = 0.0f;
	//}

	//if (glfwGetKey(window, GLFW_KEY_KP_7) == GLFW_PRESS) { //Azul
	//	Red = 0.0f;
	//	Green = 0.0f;
	//	Blue = 1.0f;
	//}
	//if (glfwGetKey(window, GLFW_KEY_KP_8) == GLFW_PRESS) { //Amarillo
	//	Red = 1.0f;
	//	Green = 1.0f;
	//	Blue = 0.0f;
	//}
